...
export const useFaceTracking = ({
  ...
  const detectAttention = (blendshapes) => {
    ...
    const lookingAway = Math.max(features.eyeLookInLeft, features.eyeLookInRight);
    const lookingDown = Math.max(features.eyeLookDownLeft, features.eyeLookDownRight);
  ...
  const renderLoop = () => {
    ...
    const loop = () => {
      ...
      if (results.faceLandmarks?.length > 0) {
        ...
        const attention = detectAttention(results.faceBlendshapes[0].categories);
